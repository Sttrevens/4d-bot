...
async def run_runtime_turn(request: RuntimeRequest) -> RuntimeResponse:
    ...
    if planned_calls:
        ...
        return RuntimeResponse(
            ...
            events=[
                {
                    "event": "runtime.checkpoint.created",
                    "run_id": request.run_id,
                    "tenant_id": request.tenant_id,
                    "payload": entry,
                }
...
def _configured_sidecar_url() -> str:
    return os.getenv("HERMES_RUNTIME_URL", "").strip().rstrip("/")


def _fetch_sidecar_health(url: str, *, timeout: int = 2) -> dict:
    req = urllib_request.Request(f"{url}/health", headers={"Accept": "application/json"})
    with urllib_request.urlopen(req, timeout=timeout) as response:
        data = json.loads(response.read().decode("utf-8"))
    if not isinstance(data, dict):
        raise ValueError("sidecar health response is not a JSON object")
    return data


def health(*, probe_sidecar: bool = True) -> dict:
    from app.hermes_runtime.observability import health_state

    sidecar_url = _configured_sidecar_url()
    if probe_sidecar and sidecar_url:
        try:
            remote = _fetch_sidecar_health(sidecar_url)
            remote["sidecar_url"] = sidecar_url
            remote.setdefault("sidecar_status", "ok")
            return remote
            ...
            return {
                ...
                "last_error": f"sidecar health probe failed: {exc}",
